...
const regular_infix_folderName_associations_ita = Dict(
    [
        "_dt2",
    ] => "andamento_della_durata_del_ricovero_prima_del_decesso__daily_deceased_hospital_length_of_stay",
    [
        "_stack_osp",
    ] => "andamento_di_ricoveri_terapie_intensive_e_decessi__daily_percentage_incidences_by_region_outcome",
...
for csv_path in readdir(archive_folder; join = true)
    ...
    elseif occursin("_bydate", csv_name) &&
        ...
        CSV.write(
            joinpath(folder_structure_root_path, "daily_incidences_by_province",
                translated_name), df)
        match += 1
    end
    ...
    if occursin("ratio", csv_name) && !occursin("ratio_age", csv_name)
        CSV.write(
            joinpath(folder_structure_root_path, "daily_incidences_ratios_by_region",
                translated_name), df)
        match += 1
    elseif occursin("ratio_age", csv_name)
        CSV.write(
            joinpath(folder_structure_root_path, "daily_incidences_ratios_by_age",
                translated_name), df)
        match += 1
    end
