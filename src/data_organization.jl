...
# Define association infix => destination folder. The "bydate" infix case is treated separately everywhere, since we have to distinguish between stratification by region and by province, the latter with rolling means
const regular_infix_folderName_associations_ita = Dict(["_dt2"] => "andamento_della_durata_del_ricovero_prima_del_decesso__daily_deceased_hospital_length_of_stay",
                                                       ["_stack_osp"] => "andamento_di_ricoveri_terapie_intensive_e_decessi__daily_percentage_incidences_by_region_outcome",
                                                       ["_quantili"] => "andamento_età_mediana__daily_age_quantiles_by_status",
                                                       ["_stack_eta"] => "andamento_per_fasce_di_età__daily_percentage_incidences_by_age",
...
const regular_infix_folderName_associations_en = Dict(["_dt2"] => "temporal_distribution_of_time_delay_from_hospitalization_to_death",
                                                      ["_stack_osp"] => "daily_incidences_percentages_by_region_outcome",
                                                      ["_quantili"] => "daily_age_quantiles_by_status",
                                                      ["_stack_eta"] => "daily_incidences_percentages_by_age",
                                                      ["_dt"] => "hospital_length_of_stay_distribution_deceased_by_region",
...
const regions_italy_names_translations = Dict("italia" => "italy",
                                              "valle_daosta" => "aosta_valley",
                                              "piemonte" => "piedmont",
                                              "lombardia" => "lombardy",
                                              "toscana" => "tuscany", "marche" => "marches",
                                              "puglia" => "apulia", "sicilia" => "sicily",
                                              "sardegna" => "sardinia")

const provinces_translations = Dict("genova" => "genoa", "milano" => "milan",
                                    "napoli" => "naples", "roma_" => "rome_",
                                    "torino" => "turin", "venezia" => "venice")

const further_translations = Dict("positivi" => "confirmed", "sintomatici" => "symptomatic",
...
## "bydate" folders. csvs with infix may have regional stratification or provincial stratification with rolling mean
...
if !isdir(joinpath(folder_structure_root_path,
                   "relative_overall_prevalences_distributed_by_sex_age"))
    mkpath(joinpath(folder_structure_root_path,
                    "relative_overall_prevalences_distributed_by_sex_age"))
end
...
const column_names_translations = Dict(
                                       ...
                                       "0-5 anni" => "0_5", "6-12 anni" => "6_12",
                                       "13-19 anni" => "13_19", "20-29 anni" => "20_29",
                                       ...
                                       "casi" => "incidence",
                                       "casi_media7gg" => "incidence_7_days_moving_average",
                                       # daily_incidences_healthcare_workers_by_region
                                       "operatori sanitari" => "healthcare_workers",
                                       "altri" => "others",
                                       ...
                                       "ricoveri" => "hospitalizations",
                                       "deceduti" => "deceased",
                                       "fascia di età" => "age_class",
                                       "deceduti/ricoveri 35gg" => "deceased_/_hospitalizations_35_days_moving_average",
                                       "dt_PRELIEVO" => "date", "positivi" => "confirmed",
                                       "deceduti/positivi 14gg" => "deceased_/_confirmed_14_days_moving_average",
                                       "dt_TERAPIAINTENSIVA" => "date",
                                       "terapia intensiva" => "intensive_care",
                                       "deceduti/terapia intensiva 42gg" => "deceased_/_intensive_care_42_days_moving_average",
                                       "ricoveri/positivi 14gg" => "hospitalizations_/_confirmed_14_days_moving_average",
                                       "terapia intensiva/positivi 14gg" => "intensive_care_/_confirmed_14_days_moving_average",
                                       "terapia intensiva/ricoveri 35gg" => "intensive_care_/_hospitalizations_35_days_moving_average",
                                       "sintomatici/positivi 14gg" => "symptomatics_/_confirmed_14_days_moving_average",
                                       # daily_incidences_ratios_by_region
                                       "terapia intensiva/ricoveri 7gg" => "intensive_care_/_hospitalizations_7_days_moving_average",
                                       "deceduti/ricoveri 7gg" => "deceased_/_hospitalizations_7_days_moving_average",
                                       "ricoveri/positivi 7gg" => "hospitalizations_/_confirmed_7_days_moving_average",
                                       # daily_incidences_percentages_by_age
                                       "0-39 anni" => "0_39", "40-59 anni" => "40_59",
                                       "60-79 anni" => "60_79",
                                       # daily_incidences_percentages_by_region_outcome
                                       "ospedalizzati" => "hospitalized",
                                       # daily_Rt_by_region_province_status
                                       "rt_positivi" => "Rt_confirmed",
                                       "rt_positivi_err68perc" => "Rt_confirmed_err68perc",
                                       "rt_sintomatici" => "Rt_symptomatic",
                                       "rt_sintomatici_err68perc" => "Rt_symptomatic_err68perc",
...
for csv_path in readdir(archive_folder; join = true)
    ...
    if "age_class" in names(df)
        df.age_class = [age_classes_translations[ita_age_class]
    ...
    for regular_infix in regular_infixes
        ...
        if all(occursin.(regular_infix, Ref(csv_name)))
            CSV.write(joinpath(folder_structure_root_path,
                               regular_infix_folderName_associations_en[regular_infix],
                               translated_name), df)
            match += 1
            break
        end
    ...
    if occursin("_bydate", csv_name) && any(occursin.(regions_italy_names, Ref(csv_name)))
        CSV.write(joinpath(folder_structure_root_path, "daily_incidences_by_region",
                           translated_name), df)
        match += 1
    elseif occursin("_bydate", csv_name) &&
           !any(occursin.(regions_italy_names, Ref(csv_name)))
        CSV.write(joinpath(folder_structure_root_path, "daily_incidences_by_province",
                           translated_name), df)
        match += 1
    end
    ...
    elseif occursin("_byage", csv_name)
        CSV.write(joinpath(folder_structure_root_path,
                           "overall_prevalences_by_region_sex_age", translated_name), df)
        match += 1
    end

    # Exceptional case "ratio": the infix may be "ratio" or "ratio_age"
    if occursin("ratio", csv_name) && !occursin("ratio_age", csv_name)
        CSV.write(joinpath(folder_structure_root_path, "daily_incidences_ratios_by_region",
                           translated_name), df)
        match += 1
    elseif occursin("ratio_age", csv_name)
        CSV.write(joinpath(folder_structure_root_path, "daily_incidences_ratios_by_age",
                           translated_name), df)
        match += 1
    end
...
const input_daily_incidences_by_region_sex_age = joinpath(input_dir_path,
                                                          regular_infix_folderName_associations_en[[
                                                                                                       "_age_date",
                                                                                                   ]])
cp(joinpath(folder_structure_root_path,
            regular_infix_folderName_associations_en[["_age_date"]]),
   input_daily_incidences_by_region_sex_age; force = true)
## Copy regional original series
const input_daily_incidences_by_region = joinpath(input_dir_path,
                                                  "daily_incidences_by_region")
cp(joinpath(folder_structure_root_path, "daily_incidences_by_region"),
   input_daily_incidences_by_region; force = true)

# Delete the national input files ended in 2_input
const national_paths = filter(path -> occursin("italy", path),
                              vcat(readdir(input_daily_incidences_by_region_sex_age;
                                           join = true),
                                   readdir(input_daily_incidences_by_region; join = true)))
