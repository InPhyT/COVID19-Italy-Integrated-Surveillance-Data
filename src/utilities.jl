...
"""
function get_aggregated_dataframe_name_from_unaggragated_dataframe(unaggregated_name::String)
...
"""
function get_over80_aggregated_dataframe_name_from_unaggragated_dataframe(unaggregated_name::String)
...
function unroll_iss_infn(iss_infn_dataframe::DataFrame, n₋::Int64, n₊::Int64,
    ...
    moving_averages::Vector{Vector{Float64}} = [[x
                                                 for x in skipmissing(iss_infn_dataframe[:,
                                                                                         age_class])]
                                                for age_class in age_classes]
    initial_conditions::Vector{Union{Nothing, Tuple{Vararg{Int64}}}} = [!isnothing(initial_conditions_dataframe) ?
    ...
    reconstructed_incidences::Vector{Vector{Vector{Int64}}} = [unroll(moving_average,
                                                                      n₋ + n₊ + 1;
    ...
    if unique(length.(reconstructed_incidences)) != [1] &&
       !isnothing(horizontal_check_cases)
        println("to_be_unrolled Dataframe has some moving averages that cannot be brought back to one and only one reconstructed time series. Lengths of reconstructions are " *
        ...
        if length(accepted_possibilities_combinations) != 1
            error("to_be_unrolled Dataframe has some moving averages that cannot be brought back to one and only one reconstructed time series, not even trying to use horizontal check, which yielded ",
